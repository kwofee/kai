    ...
    try:
        ...
        while True:
            ...
            if event == ' Log In ':
                ...
                query = 'select player_name from leaderboard;'
                cur.execute(query)
                result1 = cur.fetchall()
                query = 'select username from users where emailid = "%s";'%(gemail)
                cur.execute(query)
                ...
                for i in result1:
                    if i[0]==result2[0][0]:
                        ...
                        query = 'select userid, username from users where emailid = "%s";'%(gemail)
                        cur.execute(query)
                        result3 = cur.fetchall()
                        query = 'insert into leaderboard values(%s,"%s",%s)'%(result3[0][0],result3[0][1],0)
                        cur.execute(query)
                        con.commit()
...
def win_homepage():
        ...
        def win_howtoplay(og_win=window):
            ...
            layout = [[sg.Text('                                      How to play?', font='8514oem 20 bold',
                      ...
                      [sg.Text(
                          ...
                          font='8514oem 12')],
                      [sg.Text(
                          '7. Please note that to see changes in the leaderboard, you need to log out and log in again',
                          font='8514oem 12')],
        ...
        def win_Score(window, ball):
            og_win = window
            global ball_score2
            ball_score2 = ball.score
            ball_score = ball.score
            layout = [[sg.Text('Well Done: ', font='8514oem 16 bold', pad=(30, 10))],
            ...
            def main():
                ...
                while run:
                    ...
                    if math.ceil(ball.y) in [488, 489, 490, 491, 492, 493, 494, 495]:
                        ...
                        query = 'select userid,username,emailid from users where emailid = "%s";' % (cmail)
                        cur.execute(query)
                        result = cur.fetchall()
                        userdata = []
                        for i in result:
                            userdata.append(i[1])
                            userdata.append(cmail)
                            userdata.append(i[0])
                        query = 'select score from leaderboard where playerid = %s;' % (userdata[2])
                        cur.execute(query)
                        result = cur.fetchall()
                        for k in result:
                            oldscore = k
                        query = 'update leaderboard set score = %s' %(ball.score + oldscore[0])
                        print(userdata[2], userdata[0], ball.score)
                        cur.execute(query)
                        con.commit()
